...
const USER_EMAIL = 'learningplaywright@learning.com';
...
test("Test 1 — Banner IS visible when 6 events are returned", async ({ page }) => {
    ...
    const eventsCount = await page.locator("[data-testid='event-card']").count();
...
test("Test 2 — Banner is NOT visible when 4 events are returned", async ({ page }) => {
    //- The mock must be registered before navigating to the events page
    await page.route("**/api/events**", async (route) => await route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(FOUR_EVENTS_RESPONSE) })); 
